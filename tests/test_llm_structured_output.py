...
class LlmStructuredOutputTest(unittest.TestCase):
    ...
    def test_chat_structured_uses_json_schema_first_for_supported_provider(self, mock_post):
    ...
    @patch("llm.requests.post")
    def test_chat_structured_starts_with_json_object_for_deepseek_base_url(self, mock_post):
        mock_post.return_value = self._mock_success_response({"content": '{"answer":"ok"}'})
        client = LLMClient(
            api_key="test-key",
            model="gpt-4.1-mini",
    ...
    @patch("llm.requests.post")
    def test_chat_structured_starts_with_json_object_for_deepseek_model(self, mock_post):
        mock_post.return_value = self._mock_success_response({"content": '{"answer":"ok"}'})
        client = LLMClient(
            api_key="test-key",
            model="deepseek-chat",
