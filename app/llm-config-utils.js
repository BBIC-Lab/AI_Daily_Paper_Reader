(function (root, factory) {
  ...
  const DEFAULT_OPENAI_COMPATIBLE_CHAT_MODELS = [
    'gpt-5.4',
    'deepseek-chat',
    'qwen-plus',
    'kimi-k2.5',
  ...
  const OPENAI_COMPATIBLE_PRESETS = Object.freeze({
    ...
    openai: Object.freeze({
      ...
      models: Object.freeze(['gpt-5.4', 'gpt-4.1-mini']),
    }),
  });
  ...
  const inferProviderType = (secret) => {
    ...
    if (explicit === 'openai-compatible') return explicit;
  ...
  const buildStreamingChatPayload = ({ baseUrl, model, messages }) => {
    ...
    if (profile === 'deepseek' && normalizeText(model).toLowerCase() === 'deepseek-reasoner') {
  ...
  return {
    ...
    DEFAULT_OPENAI_COMPATIBLE_CHAT_MODELS,
