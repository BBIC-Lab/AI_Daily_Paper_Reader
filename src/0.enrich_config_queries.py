...
# - keywords 缺少 related 时，调用通用 OpenAI-compatible LLM 生成相关词
# - llm_queries 缺少 rewrite 时，调用通用 OpenAI-compatible LLM 生成英文改写
...
MODEL_NAME = os.getenv("DPR_LLM_REWRITE_MODEL") or os.getenv("DPR_LLM_MODEL") or ""
...
def main() -> None:
    ...
    client = make_task_client("rewrite", model=MODEL_NAME or None)
